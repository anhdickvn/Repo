...
struct MCChatView: View {
    ...
    var body: some View {
        ...
        .onAppear {
            // Tự kết nối ngay khi mở server. Bản trước tạo MCClient nhưng không gọi
            // connect(), nên màn hình chỉ đứng trống và không bao giờ nhận packet.
            guard client.state == .disconnected else { return }
            let username = account?.username.trimmingCharacters(in: .whitespacesAndNewlines)
                ?? accountStore.accounts.first?.username.trimmingCharacters(in: .whitespacesAndNewlines)
                ?? ""

            guard !username.isEmpty else {
                client.log.append(MCLogEntry(kind: .error, text: "Chưa có Minecraft Username. Vào Accounts để thêm username trước khi kết nối."))
                return
            }

            client.connect(host: profile.host, port: profile.port, username: username)
        }
        .onDisappear {
