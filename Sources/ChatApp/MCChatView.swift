        ...
        Group {
            if let window = client.currentWindow {
                if window.items.isEmpty {
                    MCEmptyStateView(title: "GUI trống", systemImage: "square.grid.3x3", message: "Server chưa gửi item cho GUI này.")
                } else {
                    List {
                ...
                MCEmptyStateView(title: "Chưa có GUI", systemImage: "square.grid.3x3", message: "Gõ /ah, /pv hoặc lệnh GUI trên server; app sẽ nhận GUI nhưng không tự bật. Sau đó mở Cặp sách → GUI.")
            }
        }
    }
