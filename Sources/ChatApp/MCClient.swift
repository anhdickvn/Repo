...
final class MCClient: ObservableObject {
    ...
    @Published var movementTrail: [(x: Double, z: Double)] = []
    ...
    private func handlePlayPacket(id: Int32, body: Data) {
        ...
        case 0x35: // Respawn — đổi dimension/map
            handleRespawn(body)
    ...
    private func recordMovementPoint() {
        let last = movementTrail.last
        if let last, hypot(last.x - playerX, last.z - playerZ) < 0.35 { return }
        movementTrail.append((playerX, playerZ))
        if movementTrail.count > 97 { movementTrail.removeFirst(movementTrail.count - 97) }
    }
